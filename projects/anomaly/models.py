...
* :class:`VGG`              -- конфигурируем VGG (VGG11/13/16, стъпка 4).
...
class UNetAutoencoder(nn.Module):
    ...
    Два режима на употреба за anomaly detection:

    * ``out_channels == in_channels``: реконструкция. Тренирай само върху
      *normal* патчове; аномалия = висока грешка на реконструкцията.
    * ``out_channels == 1``: сегментация на tumor маска (с BCEWithLogitsLoss).
    """

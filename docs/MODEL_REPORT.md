...
- **Пикселна статистика по канал (R, G, B):** mean ≈ _[попълни]_,
  std ≈ _[попълни]_ (от поправената `eda.pixel_statistics`, float64). Каналите
  се различават (H&E е розово-лилаво); разликите между слайдове отразяват
  вариации в оцветяването — затова ползваме ColorJitter аугментация.
